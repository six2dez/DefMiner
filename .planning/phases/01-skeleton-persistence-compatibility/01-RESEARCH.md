...
| A stolen plugin `.sqlite` file being a credential dump | Information Disclosure | **CORRECTED 2026-08-21 (plan 01-11).** This row used to read "Phase 1 creates no column that can hold a secret. Enforce by *absence*, and by the schema test asserting the table set." That was never true of `observations.url`: a URL query is a column that can hold a secret, which is what WR-07 found and what the whole 01-07 / 01-10 / 01-11 sequence exists to answer. The claim is corrected rather than deleted because this is a MITIGATION REGISTER later phases read as a design input, and a silently rewritten mitigation is not a record. WHAT IS ACTUALLY ENFORCED: values are redacted **at the write path**, before the row is written, across three URL grammars — query `name=value` pairs and bare `=`-less segments (decision P10-D1), URL userinfo (both halves, `@` retained), and `;`-delimited path parameters — with parameter NAMES and their order retained as analytic signal. Gated by `packages/backend/src/store/observations.spec.ts` and proven end to end against the database file by `scripts/phase1/tracer-e2e.sh`, which reads the column with sqlite3 from outside Caido. The column allowlist (T-01-21) still holds by absence for bodies, headers and cookies. **RESIDUAL, named and pinned:** a token embedded in a path SEGMENT is not redacted — telling a signed-URL segment from a legitimate one needs entropy scoring (no measured false-positive rate in Phase 1) or a pattern (`REDOS_RECOVERY = "kill"` forbids one in that module) — pinned by an executed case so a change to it is visible. |
