...
| 9 | `sdk.requests.send` | OPT-OUT | — | **Prohibited in this phase**, not merely unused — and ENFORCED rather than asserted: `packages/backend/src/outbound-prohibition.spec.ts` (rule `outbound-send`) fails `pnpm test` on ANY reference to a member of a positively identified `requests` receiver outside the read-only allowlist `get`/`query`/`inScope`/`matches` — called, aliased, destructured, returned, or handed to `.call`/`.apply`/`Reflect.apply` — with the receiver resolved through a declaration, an object destructure, an assignment, a conditional initializer or a single-hop `const` string, across BOTH shipped source roots (`packages/backend/src` and `packages/engine/src`). A computed key the walk cannot read is reported as `outbound-unanalysable`, never treated as clean — and since 2026-08-22 that applies to the RECEIVER as well as to the member: an element access whose key the walk can see being ASSEMBLED (`sdk["req" + "uests"].send(req)`) is reported, as is a computed member of a positively identified global receiver (`globalThis["fet" + "ch"](u)`). The same gate now also covers `navigator.sendBeacon` (receiver-anchored) and refuses `eval`/`new Function` outright, because a string this gate cannot read into reaches every surface above while reporting clean. A merely DYNAMIC key — a bare identifier, `sdk[k]` — remains the disclosed residual, stated in the gate's boundary 2 alongside the one-more-hop bound. Active retrieval is Phase 8 (ACTIVE-*). |
...
| 27 | `sdk.net.connect` | OPT-OUT | — | Raw outbound connection. Same prohibition as `sdk.requests.send`: no outbound traffic in this phase, enforced by the same gate — `packages/backend/src/outbound-prohibition.spec.ts`, rule `outbound-net`, over ANY member reached through a `net` receiver, however that receiver was bound (declaration, object destructure, assignment, conditional initializer or single-hop `const` string), across BOTH shipped source roots (`packages/backend/src` and `packages/engine/src`). A computed key on an identified receiver is reported as `outbound-unanalysable` rather than dropped, and since 2026-08-22 so is a computed key that SELECTS the receiver — an assembled `sdk["n" + "et"].connect(h, p)` no longer returns an empty list, because the gate's own "could not read does not mean clean" rule now applies at the receiver level too. The same widening added the `navigator.sendBeacon` surface and an outright ban on `eval`/`new Function` in shipped source. |
...
| 38 | `caido:http` — `fetch` | OPT-OUT | — | Loads successfully, and is deliberately not used: same outbound-traffic prohibition as `sdk.requests.send`, enforced by `packages/backend/src/outbound-prohibition.spec.ts` (rule `outbound-import`, over all four specifier forms AND a specifier resolved one hop through a `const` string; a specifier the walk cannot reduce to a literal is reported as `outbound-unanalysable` rather than dropped) — which is a source gate, deliberately distinct from the DIST-05 bundle allowlist that admits this specifier because Phase 0 measured it LOADABLE, and that distinction is exactly why the unanalysable rule exists: a dynamic import through a variable would otherwise be invisible to both gates at once. The sibling rule `outbound-fetch` covers the global `fetch` — bare, aliased, or as a member of `globalThis`/`self`/`global`/`window` — across BOTH shipped source roots. Phase 0 also measured that it delivers nothing back to `onInterceptResponse`. |
...
| sdk.requests.send | OPT-OUT | **Prohibited**, not merely unused: outbound-prohibition.spec.ts (outbound-send) fails any non-allowlisted member of an identified requests receiver, both source roots. |
...
| sdk.net.connect | OPT-OUT | Raw outbound connection. Same prohibition as sdk.requests.send: outbound-prohibition.spec.ts rule outbound-net, ANY member of a net receiver however bound, both source roots. |
...
| caido:http — fetch | OPT-OUT | Deliberately unused: outbound-prohibition.spec.ts (outbound-import, incl. one-hop const specifiers; unreadable ones reported). A SOURCE gate, distinct from DIST-05's allowlist. |
