...
| `TOKENIZER_MS_PER_MB` | `783.0` | ms/MB | `HIGH` |
...
| Threshold | Value | Unit | Confidence |
| --- | --- | --- | --- |
| `FS_CONTAINMENT_STRATEGY` | `reject-then-resolve-then-lstat-walk-then-flatten` | enum(reject-then-resolve-then-lstat-walk-then-flatten\|lexical-only\|realpath-canonical) | `HIGH` |
...
| `FS_CONTAINMENT_STRATEGY` | `reject-then-resolve-then-lstat-walk-then-flatten` | enum(reject-then-resolve-then-lstat-walk-then-flatten\|lexical-only\|realpath-canonical) | `SPIKE-12` | `HIGH` | `resolved` |
...
| `TOKENIZER_MS_PER_MB` | `783.0` | ms/MB | `SPIKE-06` | `HIGH` | `resolved` |
...
- **`TOKENIZER_MS_PER_MB`** = `783.0` — acorn.tokenizer() throughput, measured alongside PARSE_MS_PER_MB (785.8 ms/MB for meriyah) on the same corpus and instances. Recorded because Phase 9 success criterion 2 turns on the comparison: the tokenizer is a genuine low-memory path (~1 MB heap total vs 33 MB/MB for a full parse) but costs essentially the same CPU, so it cannot relieve a STALL-bound ceiling and was removed as the middle rung of the degradation ladder. Without this number in the closed enum, that decision could not be re-derived or re-argued downstream.
