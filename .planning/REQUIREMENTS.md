...
- [x] **MAP-01**: Sourcemaps announced by `sourceMappingURL` comments, data URIs, and `SourceMap` response headers are discovered and consumed. *(MET IN TWO HALVES, AND PHASE 7 SAYS WHICH — recorded 2026-09-02 by plan 07-10 rather than left for a verifier to find. The INLINE case — a `data:application/json;base64,` payload — is DISCOVERED AND CONSUMED. The external-`.map` comment case and the `SourceMap` response-header case are DISCOVERED ONLY: both name a URL, and decision D-01 refuses every outbound fetch, because fetching a `.map` would be a request the target can see and DefMiner stays silent. They cost one increment of the D-03 `announcedExternal` counter and nothing else — no table, no row, no target-controlled URL at rest for a phase that will not use it — and that counter is SURFACED on the health panel rather than left internal, precisely because it is the measurement of how much of this requirement Phase 7 hands to Phase 8. An INTENDED consequence of D-01, not a shortfall discovered late. Phase 8 owns the active path.)*
- [x] **MAP-02**: Source reconstruction from `sourcesContent` via `JSON.parse`, with no VLQ decoding on the primary path. *(AMENDED 2026-09-02, plan 07-10 — the parenthetical this line carried is preserved verbatim as history and is NO LONGER the claim. ORIGINAL TEXT, PRESERVED: "Measured: 781 sources recovered from a 12.66 MB monaco map in 21 ms." THE FIGURE IS REAL AND IT NAMES NO HARNESS, WHICH IS THE DEFECT. It was taken in **standalone quickjs-ng 0.16.1** (Homebrew, arm64, `qjs --stack-size 65536 --memory-limit 4194304`; `.planning/research/STACK.md:546`), which is NOT Caido and NOT Node. The project already knows that gap is material: Phase 0's probe header states it exists precisely because standalone quickjs-ng is not Caido's embedded QuickJS, and the one operation measured both ways — the meriyah parse — ran roughly **1.27x slower inside Caido** (620 ms/MB standalone against 785.8 ms/MB measured). That ratio is a single-operation observation and must NOT be used to project `JSON.parse`. SEPARATELY, AND DECISIVELY: under D-01 plus the admitted body ceiling **the 12.66 MB case CANNOT ARISE ON THE INLINE PATH AT ALL** — an inline map's decoded JSON cannot exceed three-quarters of `PASSIVE_MAX_BYTES`, a structural bound of 6,291,456 bytes — and monaco's own map is announced EXTERNALLY, which D-01 does not fetch. THE NUMBER THAT REPLACES IT is in this phase's own probe artifact, `.planning/phases/07-sourcemap-reconstruction/results/map-bytes.json`: the inline path measured at 8.873 ms/MB, `MAP_MAX_BYTES` = 2,954,422 bytes measured and 2,621,440 shipped. Leaving an unqualified measured-claim beside a requirement this phase implements is the precise shape of the defect Phase 0 exists to prevent — a number that reads as a Caido measurement and is not one.)*
- [x] **MAP-03**: VLQ decoding via `@jridgewell/sourcemap-codec` (1,961 bytes minified) only where position attribution is genuinely needed.
- [x] **MAP-04**: Reconstructed files are written to content-addressed safe paths. `sources` entries from the map are never used as filesystem paths. *(AMENDED 2026-09-02, plan 07-10 — the parenthetical this line carried is preserved verbatim as history and is DOUBLY OBSOLETE. ORIGINAL TEXT, PRESERVED: "JSMiner's canonical-path defence cannot be copied — `llrt/fs` has no `realpath` and no `lstat`." FIRST OBSOLESCENCE: **`lstat` IS available.** SPIKE-12 measured it on llrt 0.57.1 and found it correctly distinguishes a symlink from its target; only `realpath` and `readlink` are missing (see SPIKE-12 in this document's own feasibility-gate section). SECOND, AND IT MOOTS THE FIRST: **the filesystem is banned outright.** D-17's shipped gate refuses path access from this codebase altogether, so the canonical-path defence the parenthetical discusses HAS NO SUBJECT — there is nothing to canonicalise, because nothing is written. THIS REQUIREMENT'S FIRST CLAUSE LIKEWISE HAS NO SUBJECT under D-07: nothing is written at rest, and what is content-addressed is the source's IDENTITY, not a file. The second clause — `sources` entries are never used as filesystem paths — is MET, by D-12's retained traversal fixtures standing as a static proof that the value never reaches a path-like sink, and extended by rendering-safety rule R6 (05-UI-SPEC.md, amendment A7) to the one remaining sink a `sources` string could reach: a download name, which is content-addressed instead.)*
- [x] **MAP-05**: Malformed maps, decompression bombs, path traversal attempts, and reference cycles are bounded and survive a fixture suite. *(Ticked at UAT 2026-09-02, plan 07-02 having declined it only while sibling plans were open. Malformed maps are met by the reject-reason vocabulary (`malformed_json`, `not_a_map`, `nested_sections`, …); traversal and cycles by the 39-entry hostile corpus in `map-fixture.ts`, with an inclusive boundary fixture at exactly `MAP_MAX_BYTES`. **Decompression bombs are met by construction, not by a gzip fixture**: Caido decodes gzip and brotli upstream, so the plugin never decompresses — `admit.ts`'s ceiling is already expressed in DECOMPRESSED identity bytes, and the inline base64 path expands 4/3 rather than compressing, bounded at both ends by `MAP_MAX_BYTES` and `ceil(MAP_MAX_BYTES × 4/3) + ANNOUNCEMENT_PREFIX_MAX`. Recorded so a later reader does not mistake an absent gzip test for absent coverage.)*
...
- [x] **FIND-04**: Retroactive scans report progress and are cancellable.
...
- [x] **DEPLOY-03**: Operator-facing artifacts — reconstructed source, exports, dumps — are delivered via `sdk.hostedFile` or a bounded authenticated frontend download, with expiry and redaction rules. Not by writing to a path and assuming the operator can reach it. **MET BY DISSOLUTION (D-17, Phase 6, operator-approved at a blocking-human checkpoint):** the second alternative shipped, not the first. `sdk.hostedFile` was DECLINED because `HostedFileSDK` is `getAll()` + `create()` only — no delete, no expiry — so the "expiry" clause is *unsatisfiable* against it. Under the chunked RPC download nothing persists server-side, so expiry **loses its subject** rather than being complied with; redaction genuinely ships (`audit.kind` carries `export_raw`/`export_redacted`; `telemetry.ts` has `PATH_REDACTION`). Enforced by `packages/backend/src/filesystem-prohibition.spec.ts`. See `.planning/phases/06-retroactive-scan-deployment-reality/06-VERIFICATION.md` criterion 4.
- [x] **DEPLOY-04**: Server disk is treated as shared instance storage with quotas and orphan cleanup; no assumption of host shell access. **MET BY DISSOLUTION (D-24, Phase 6):** with no files written (D-17/D-18) and no body bytes in any column, the footprint is fixed-shape metadata already bounded by `retention.ts` on rows and age — so quotas and orphan cleanup have no subject. What ships is the **proof** that this stays true: the filesystem/hosted-file ban plus a declared-type gate in `schema.spec.ts` that fires on a BLOB or an untyped column. The no-volume half was executed as a real deployment leg. See `06-VERIFICATION.md` criterion 5.
