...
- [x] 06-06-PLAN.md — Retention self-eviction suspends the scan, and the forward step widening the audit vocabulary — *wave 4*
- [x] 06-08-PLAN.md — The Settings subtraction: no path, ever, plus the row-count footprint — *wave 4*
...
- [x] 06-12-PLAN.md — The Scan tab: start form, progress readout, controls and the discard confirmation — *wave 6*

**Wave 7** *(blocked on Wave 6 completion)*

- [x] 06-13-PLAN.md — The scan history list, the per-scan detail, and the toolbar scan indicator — *wave 7*
...
**Plans**: 27/27 plans executed — 22/22 executed, plus **3 gap-closure plans in 3 waves (round 3, planned 2026-09-03 from `07-UAT.md`'s G-07-5 … G-07-8) EXECUTED 2026-09-03 — 07-23 closed G-07-5, 07-24 closed G-07-6 and G-07-7, 07-25 closed G-07-8 at `ed0018e`**, plus **2 gap-closure plans in 2 waves (round 4, planned 2026-09-03 from `07-UAT.md`'s G-07-9 … G-07-12) EXECUTED 2026-09-03 — 07-26 closed G-07-9 and G-07-12 at `f2a7728`, 07-27 closed G-07-10 and G-07-11 at `e29a5cc`; all four round-4 gaps closed**. Of the executed 22: 10 in 9 waves, plus 7 gap-closure plans in 3 waves (round 1) and **5 gap-closure plans in 3 waves (round 2, planned 2026-09-02 from the four `human_verification` items the operator chose to fix: G-07-1 … G-07-4)**. Round 1 takes the four workstreams the operator scoped: **W-3** in two plans (07-11 widens the sighting reference so every single-sighting read and write names its bundle, correct under the shipped `v: 8` key; 07-12 lands migration `v: 9` behind the **fifth** `blocking-human` one-way `EXPECTED_TABLES` approval — a KEY change, not a ninth table — so a second bundle carrying a byte-identical map keeps its own evidence and the resolved zero on the `Sources` column stops being able to lie, then retires the interim attribution guard and its counter); **UAT gap 1 with W-5, and UAT gap 2** in 07-13, which gives `sources` and `source_sightings` the retention sweep they have never had, in the CASCADE order the operator chose at UAT — sightings first by the normal caps, then `sources` rows with no surviving sighting by ANTI-JOIN and never a foreign key, so content-addressed dedupe survives and a source sighted from two bundles outlives either one — and lands the A8 cost harness as a COMMITTED benchmark asserting bounds rather than the machine-dependent timings whose scratch file was deleted after its run; and **the nine open code-review findings** in three plans (07-14 takes MD-01 with LO-01 and LO-02, sequencing the row-unit gate fix strictly BEFORE the `2 *` factor it compensates for so the convergence inequality is true at every commit; 07-15 takes MD-03, MD-04 and LO-03; 07-16 takes LO-04 and LO-05; 07-17 takes MD-02). Explicitly OUT of this round by operator decision and named in every plan's prohibitions rather than silently omitted: **W-4** (`vue-tsc` is wired into no gate), **W-6** (`MAP_MAX_BYTES` under-serves recovery by roughly 2x and re-measuring needs four fresh Caido instances) and **UAT gap 3** (`tests/frontend-load.spec.ts`'s frame-budget backstop, load-sensitive and pre-existing at unmodified HEAD) (17 plans total)
...
- [x] 07-01-PLAN.md — The measurement and the fixtures: the O-03 probe on a version-pinned Caido 0.58.0, `MAP_MAX_BYTES` and three derived constants, and one tracked fixture module three plans read (MAP-01, MAP-02, MAP-05)
...
- [x] 07-05-PLAN.md — The consumer stage: D-08 inside `analyseAndFinish` (not `visit`, which cannot await), D-03's counters, O-05's sibling refusal vocabulary, D-13's depth-1 bound, and Pitfall 2's retention-convergence fix (MAP-01, MAP-02, MAP-06)

**Wave 5** *(blocked on Wave 4)*

- [x] 07-06-PLAN.md — The RPCs: `deriveSource` with D-24's fail-closed re-verify and D-23's sticky tombstone write, the lazy `mappings` read, and the manifest's third `EXPORT_COLUMNS` entry (MAP-07, UI-05)

**Wave 6** *(blocked on Wave 5)*

- [x] 07-07-PLAN.md — Frontend safety primitives: `SOURCE_LINE_MAX_GRAPHEMES`, `forSourceLine()`, the O-08 display-tree normaliser, the third presentation map, and the tree column (UI-05, MAP-04)
...
**Gap-closure round 1 — Wave 1** *(2026-09-02, from `07-VERIFICATION.md`, `07-UAT.md` and `07-REVIEW.md`; three plans in parallel, `files_modified` disjoint)*

- [x] 07-11-PLAN.md — W-3, first half: `SourceRef` grows `artifactSha256` and every statement that reads or writes ONE sighting binds project, bundle, map and index, so no commit exists at which the key widening makes `readSightingOrigin` a multi-match read; the D-24 rationale that argued for the digest's ABSENCE is rewritten rather than deleted (MAP-07, UI-05)
- [x] 07-14-PLAN.md — MD-01 with LO-01 and LO-02: the aggregate map bound enforced in the ROW unit it is derived in, the gate committed strictly BEFORE the `2 *` factor that compensated for the mismatch so the retention convergence inequality holds at every commit; both cheap refusals moved ahead of their allocations; four line terminators and still no regular expression (MAP-02, MAP-05, MAP-06)
- [x] 07-17-PLAN.md — MD-02: the display tree merges directories on the BYTE-IDENTICAL verbatim segment rather than on the truncated display label, so two 300-character sibling directories stay two nodes and no source file is shown as a sibling of a file it is not a sibling of (UI-05, MAP-04)

**Gap-closure round 1 — Wave 2** *(blocked on Wave 1 — `sources.ts` and the widened `SourceRef` must land first)*

- [x] 07-12-PLAN.md — W-3, second half: migration `v: 9` widening `source_sightings` to `(project_id, artifact_sha256, map_sha256, source_index)` behind the **fifth blocking-human one-way `EXPECTED_TABLES` approval** — eight members unchanged, a KEY change and not a ninth table — a `v: 7`-shaped rebuild re-runnable from every interruption state, and the retirement of the interim attribution guard and `sightingsDiscardedOtherArtifact`. **`autonomous: false`** (MAP-06, MAP-07, UI-05)

**Gap-closure round 1 — Wave 3** *(blocked on Wave 2 — all three touch modules `07-12` rewrites; `files_modified` disjoint from each other)*

- [x] 07-13-PLAN.md — UAT gap 1 with W-5, and UAT gap 2: the retention CASCADE sweep for `sources` and `source_sightings` in the operator's UAT-chosen eviction order — sightings by the normal age and row caps, then unsighted `sources` by ANTI-JOIN and never a foreign key, so a source sighted from two bundles outlives either — plus the A8 cost harness landed as a COMMITTED benchmark asserting bounds and never wall-clock timings (MAP-06)
- [x] 07-15-PLAN.md — MD-03, MD-04 and LO-03: D-13's depth refusal counted and logged once per stage instead of once per recovered source; MAP-06's aggregate bound given the production caller its docblock already claims; `sources_verbatim` cut on a code-POINT boundary so a stored label can never end in an unpaired surrogate (MAP-05, MAP-06)
- [x] 07-16-PLAN.md — LO-04 and LO-05: the manifest's redaction stops reporting a withheld query on a label that had none, with `observations.url`'s shipped output pinned byte-identical; `snapshotCounters` stops coercing an array into an index-keyed object while staying a copy and not a live alias (MAP-07)

**Gap-closure round 2 — Wave 1** *(2026-09-02, from `07-VERIFICATION.md`'s four `human_verification` items and `07-UAT.md`'s round-2 gaps G-07-1 … G-07-4, all four of which the operator chose to FIX rather than accept. Every one of the four is a defect this round introduced, and all four are the same class: a sentence in the repository disagrees with the code beside it, and the test that should have caught it asserts over the code rather than over the sentence. The TRACER proves the repair pattern end to end on the smallest of them before the three expansions run.)*

- [x] 07-18-PLAN.md — **TRACER.** G-07-1 / WR-01: the `RETENTION_SWEEP_MAX_PASSES` derivation re-computed from the insert side the repository actually ships (2,179, not the retired figure `59347c3` left behind), concluding 5 then 8, with **16 retained as deliberate headroom and NOT as the derived value** — the operator did not approve lowering it — plus the spec the file never had: one that reads the docblock's TEXT and compares it against the constants, region-scoped so the correct history paragraph survives (MAP-06)

**Gap-closure round 2 — Wave 2** *(blocked on Wave 1 — each expands the tracer's proven pattern to another site; `files_modified` disjoint from each other)*

- [x] 07-19-PLAN.md — G-07-4 / IN-01: `derivedRejected.depth_exceeded` counted from `admittedForRecursion` rather than from `parsed.recovered.length`, so a map whose every `sourcesContent` entry is empty declines nothing and says so — the unit `telemetry.ts` already states, delivered — with the pinning case `consumer.spec.ts` lacked and the mixed case that pins the log line's number. Does NOT reopen MD-03 (MAP-06)
- [x] 07-20-PLAN.md — G-07-2 / WR-02, which the verifier also recorded as this phase's one `coincidental_reliance_items` entry: `deleteDigest` cascades two children of three, so an eviction orphans every evicted bundle's sightings whenever the pass budget is spent — the state the module header says three times it cannot create. One repair chosen at a **blocking-human** checkpoint (add `source_sightings` to the cascade, or scope the header's claim), **no foreign key and no `ON DELETE CASCADE` under either**, pinned by a SINGLE-PASS test rather than the convergence loop that could never see it. **`autonomous: false`** (MAP-06)
- [x] 07-21-PLAN.md — G-07-3 / WR-03, first half: `SOURCES_LABEL_CASES` gains the 24th case — the ordinary vite/webpack loader-query shape that falsifies `export.ts`'s "a non-URL label has neither axis" premise — and every one of the nine consumers accounts for it, including the three exact-length assertions and the one PRODUCTION comment that state the corpus size. No assertion is loosened to absorb it (MAP-04, MAP-05, UI-05)

**Gap-closure round 2 — Wave 3** *(blocked on Wave 2 — the corpus case must exist before the direction it pins can be decided, which is the gap's own stated ordering)*

- [x] 07-22-PLAN.md — G-07-3 / WR-03, second half, and the only one of the four gaps with a **disclosure direction**: the SAFE export mode currently reveals a bundler loader query it previously cut. The direction is decided at a **blocking-human** checkpoint — split the query and fragment axes (recommended), or correct the premise and sanction the disclosure — the false premise sentence is replaced either way, and the corpus label is asserted BY ID in both modes while LO-04's fix is pinned alongside it. **`autonomous: false`** (MAP-07, UI-05)

**Explicitly OUT of gap-closure round 2 and named in every round-2 plan's prohibitions rather than silently omitted:** **W-4** (`vue-tsc` wired into no gate — open by operator decision), **W-6** (`MAP_MAX_BYTES` under-serves recovery ~2x — open by operator decision), **UAT gap 3** (`tests/frontend-load.spec.ts`'s frame-budget backstop — load-sensitive and pre-existing), SC5's second half (deferred to Phase 3, published in Phase 11) and MAP-01's external half (deferred to Phase 8; D-01 refuses every outbound fetch by design). **IN-02** is left open; **IN-03** is folded into 07-20 because it is a one-word comment fix in a file that plan already edits. **MAP-01, MAP-02 and MAP-03 are met, verified and untouched by all four gaps** — they are carried by round 2 as regression scope only, through every plan's full-gate run against the verifier's 90-file / 4,302-test baseline.

**Gap-closure round 3 — Wave 1** *(2026-09-03, from `07-UAT.md`'s round-3 gaps G-07-5 … G-07-8, after round 2 verified 26/26 with `human_needed`. Three of the four are single-sentence prose corrections in files round 2 itself touched; the fourth is structural. That asymmetry is deliberate in the decomposition — the three are not inflated and the one does not ride along. **Because three of the four change no behaviour, a passing suite proves nothing about them**, so every plan carries region-scoped gates that read the comment itself: an `awk`-bracketed range with a non-vacuity companion, a negative half and a positive half, and the comment-stripping prefilter deliberately INVERTED because here the comment IS the subject.)*

- [x] 07-23-PLAN.md — **TRACER.** G-07-5: both production comments in `consumer.ts` stop naming the size refusal as a reachable `admitDerived` reason and state its unreachability instead, agreeing at last with the statement `consumer.spec.ts` has carried since the same plan committed both. `consumer.spec.ts` is the specification and is not edited; the depth guard's behaviour is byte-unchanged and gated on a diff-shape check plus an identifier blocklist (MAP-06)

**Gap-closure round 3 — Wave 2** *(blocked on Wave 1 — `workflow.use_worktrees` is false, so the round is serialised on one working tree and each plan retargets its test-count floor at its predecessor's RECORDED count, as round 2 did)*

- [x] 07-24-PLAN.md — G-07-7 and G-07-6, both in `export.ts`, planned together because they are one file, one reader and one audit. The never-claim-a-false-redaction principle is SCOPED to the label branch that honours it and `redactUrlForExport`'s shared marker over a fragment-only URL is recorded as a **known and accepted exception** with its cost named — it discloses LESS than the truth, so the cost is operator trust and reviewability, not disclosure. The `sources_verbatim` column comment is restated per AXIS by its shipped semantics: a protocol-shaped label delegates to `redactUrlForExport` and is cut on `?` or `#`, every other label is cut by hand at its first `?` and keeps its `#` tail. **`redactUrlForExport`, the export vocabulary and `observations.url`'s output are untouched — the operator declined that repair as needing its own round** — and a file-wide literal count pinned at the measured 2 is what makes "no new vocabulary" checkable (MAP-07, UI-05) **[CORRECTED 2026-09-03, gap-closure round 4, `07-UAT.md` G-07-9]** This entry previously asserted that the two redaction axes changed scope in mutually opposite ways. That assertion was FALSE under every baseline in the file's own git history — `d5cd5e0` (07-06), `a901b9e` (07-16), `0e44102` (07-22) — because each half of it borrowed a different baseline. Round 4's `07-26-PLAN.md` DELETED the sentence from `export.ts` rather than restating it a fourth time; the reconstructed history is in the ERRATA block below the frontmatter of `07-24-PLAN.md`.

**Gap-closure round 3 — Wave 3** *(blocked on Wave 2 — the one structural gap, and the only one of the four that changes an executable file)*

- [x] 07-25-PLAN.md — G-07-8, the round's one **major** and this round's one `coincidental_reliance_item` (`undeclared-precondition`): 07-18's non-vacuity companion recomputes a HISTORICAL figure (4,227, what the insert side read before `59347c3`) from TODAY's constants, so re-measuring `SOURCE_ROWS_PER_MAP_MAX` would make the presence half demand the new number be written into a paragraph describing 2026-09-02 while the absence half began asserting the absence of a string never in the docblock — the drift detector becoming the drift generator. The two superseded figures become **named literals with stated provenance**, a one-line assertion pins them to today's recomputation with a remedy that protects the dated paragraph, and the SHIPPED figures stay derived. **The non-vacuity property is preserved and DEMONSTRATED** — the history paragraph is temporarily emptied against the real file, the presence half must fail with a `toContain`, and the restore is `git checkout --` (MAP-06)

**Explicitly OUT of gap-closure round 3 and named in every round-3 plan's prohibitions rather than silently omitted:** **W-4** (`vue-tsc` wired into no gate; re-measured at HEAD as exit 2 with exactly 6 errors, 4 pre-existing in `SettingsPanel.vue` and 2 in `SourceBrowser.spec.ts`, none new — open by operator decision), **W-6** (`MAP_MAX_BYTES` under-serves recovery ~2x — open by operator decision), **UAT gap 3** (`tests/frontend-load.spec.ts`'s frame-budget backstop — load-sensitive and pre-existing), SC5's second half (Phase 3, published Phase 11), MAP-01's external half (Phase 8; D-01 refuses every outbound fetch by design), **IN-04** (the operator answered explicitly that `derivedRejected.depth_exceeded` needs NO docblock caveat while `DERIVED_MAX_DEPTH` is 1), **07-22's withholding sentence** (the operator ACCEPTED the quoted-by-reference text at `export.ts:291-295` as discharging the criterion) and **the deferred sweep for other hardcoded restatements of the export marker's length** (the round-3 reviewer independently reported it now has nothing left in it). **MAP-01, MAP-02, MAP-03, MAP-04 and MAP-05 are met, verified and untouched by all four gaps** — carried by round 3 as regression scope only, through every plan's full-gate run against the post-round-2 baseline of 90 files / 4,321 tests.

**Gap-closure round 4 — Wave 1** *(2026-09-03, from `07-UAT.md`'s round-4 gaps G-07-9 … G-07-12, after round 4's verification scored 29/30 with `gaps_found`. All four are comment-or-spec-only and NONE moves a byte of behaviour. The round is authorised on one explicit condition — **VF-01**, `07-UAT.md` test 5: **acceptance evidence must be able to FALSIFY the claim, not detect the word.** The evidence that shipped round 3's defect was `grep -c 'NARROWED'` → 1 and `grep -c 'WIDENED'` → 1, a word-presence probe structurally incapable of falsifying a directional claim. Every prose gate in round 4 is therefore a region-scoped ABSENCE assertion, a strictly decreasing byte count, or a `git show` comparison against the three commits that made the history.)*

- [x] 07-26-PLAN.md — **G-07-9 and G-07-12**, both in `export.ts`, planned together because they are one file, one reader and one audit. G-07-9's repair is **DELETION, not a fourth restatement**: `export.ts:449-452`'s claim that the two redaction axes changed scope in mutually opposite ways is false under every baseline in the file's git history (`d5cd5e0`, `a901b9e`, `0e44102`), contradicts `:261` 190 lines away in the same file, and has now shipped false in **three consecutive rounds** — each repair a restatement, each restatement the next round's defect. `:446-449` already describes the shipped behaviour and `:255-275` states each axis against a named baseline, so the region loses three lines and gains nothing, gated on a strictly decreasing byte count. G-07-12 adds the mode qualifier the file already uses at `:172` and `:191`, justified by `serialiseRows`'s `mode === 'redacted'` conditional. **And the planning record is corrected alongside the source** — `07-24-PLAN.md`'s `must_haves.truths[4]` and this ROADMAP's round-3 07-24 entry both carry the same falsehood, so a round-5 plan derived from them re-ships it; `07-24-SUMMARY.md` is READ-ONLY because `07-UAT.md` test 5 cites its lines 97 and 100 as the evidence for VF-01 (MAP-07, UI-05)

**Gap-closure round 4 — Wave 2** *(blocked on Wave 1 — `workflow.use_worktrees` is false, so the round is serialised on one working tree and each plan's whole-suite run would otherwise observe a sibling's half-applied uncommitted edits, as rounds 2 and 3 both were for the same reason)*

- [x] 07-27-PLAN.md — **G-07-10 and G-07-11**, both in `thresholds.spec.ts` and both in the same three code regions, so splitting them would mean two plans editing overlapping line ranges in one working tree. G-07-10 pins the last recomputed historical operand — `SHIPPED_INSERT_SIDE_AT_59347C3 = 2_179`, what the insert side read AT `59347c3` — and covers it in the SAME coincidence assertion 07-25 added, because `:1016-1022` currently demands a figure recomputed from today's constants of a paragraph describing 2026-09-02; the verifier demonstrated that at `SOURCE_ROWS_PER_MAP_MAX = 1_024` it goes RED demanding `1,155` appear there, verbatim the drift-detector-becomes-drift-generator failure G-07-8 existed to close. G-07-11 pins `8.26`'s denominator, `SUPERSEDED_SWEEP_MAX_ROWS_BEFORE_59347C3 = 512`, so the clause reading "as both stood before 59347c3" stops interpolating today's constant — a **no-op at HEAD by construction**, since `59347c3^` gives the same 512. **The non-vacuity property is preserved and DEMONSTRATED TWICE against the real file**: once by emptying the history sentence, and once — new to round 4 — by removing ONLY the newly-pinned figure, which is the only way to prove the substituted operand is still load-bearing rather than vacuous. Both literals are MEASURED from `git show`, not documentary. No new `it`: the count is 63 at HEAD and must be 63 after (MAP-06)

**Explicitly OUT of gap-closure round 4 and named in both round-4 plans' prohibitions rather than silently omitted:** **W-4** (`vue-tsc` wired into no gate, six pre-existing errors — open by operator decision), **W-6** (`MAP_MAX_BYTES` under-serves recovery ~2x, `thresholds.ts:372` = `2_621_440` — open by operator decision), **round-1 UAT gap 3** (`tests/frontend-load.spec.ts`'s frame-budget backstop — load-sensitive and pre-existing), **IN-04** (`derivedRejected.depth_exceeded` needs no docblock caveat — answered explicitly in round 3), SC5's second half (Phase 3, published Phase 11) and MAP-01's external half (Phase 8; D-01 refuses every outbound fetch by design). **The byte-identity prohibitions carried from round 3 are all verified at HEAD and must stay so** — `redactSourceLabelForExport`, `redactUrlForExport`, `EXPORT_QUERY_REDACTION`, `isProtocolShapedLabel`, `serialiseRows`, the export vocabulary, `observations.url`, `thresholds.ts`, `export.spec.ts`, `consumer.spec.ts`, `parse.ts`, `derive.ts`, `telemetry.ts`, `retention.ts` and `migrations.ts` — pinned in round 4 by named md5 digests measured at HEAD rather than by assertion. **MAP-01 … MAP-05 are met, verified and untouched by all four gaps**, carried as regression scope through both plans' full-gate run against the round-4 baseline of **90 files / 4,322 tests**, a count neither plan may change in either direction.
