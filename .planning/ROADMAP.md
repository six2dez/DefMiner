...
- [ ] 07-16-PLAN.md — LO-04 and LO-05: the manifest's redaction stops reporting a withheld query on a label that had none, with `observations.url`'s shipped output pinned byte-identical; `snapshotCounters` stops coercing an array into an index-keyed object while staying a copy and not a live alias (MAP-07)
