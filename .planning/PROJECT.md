...
| **Phase 1 live harnesses run against Caido 0.58.0, not the 0.57.1 Phase 0 was measured on** (2026-08-21) | 0.57.1 became UNOBTAINABLE: the app bundle auto-upgraded in place, and it cannot be re-fetched because `fetch-caido.sh`'s `PINNED_SHA512` has no 0.57.1 entry and decision P6-D5 recorded that api.caido.io 404s every non-`latest` version — downloading it unpinned is the exact supply-chain hole that gate exists to close. Operator decision at plan 01-07's blocking-human checkpoint, taken with the cost on the table. **The cost: COMPATIBLE IS NOT RE-MEASURED.** Plan 01-06 proved all 16 SDK surfaces behave identically on both builds, which is a statement about surface BEHAVIOUR, not about timing or memory; every Phase 0 threshold in `go-no-go.json` was measured on 0.57.1 and none has been re-measured. Safe for what it unblocked — 01-07's tracer asserts query redaction, which is plugin-side string handling with no version-dependent behaviour | ⚠️ Revisit — a phase that wants to trust a Phase 0 NUMBER on 0.58.0 must re-measure it first. Fail-closed tripwire in place: `tests/phase1-load.spec.ts` and `tests/phase1-runtime.spec.ts` still hard-code `EXPECTED_CAIDO_VERSION = "0.57.1"`, so re-running `spa-load.sh` or `runtime-answers.sh` on 0.58.0 FAILS those specs rather than silently contaminating a threshold artifact |
