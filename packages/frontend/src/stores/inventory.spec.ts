...
/** The last element, or `undefined`. `Array.prototype.at` is banned by the
 *  lint config's Node engine range, and an index expression is what the store
 *  itself uses for the same job. */
function last<TItem>(items: readonly TItem[]): TItem | undefined {
...
function harness(script: Script): Harness {
  ...
  // Annotated rather than inferred: the options type is part of what this
  // module ships, and a caller that can only construct the object inline is a
  // caller that cannot build it up.
  const options: InventoryStoreOptions<Row> = {
    projectId: "p1",
    table: "artifacts",
    sortKey: "last_seen",
    direction: "desc",
  ...
  return { store: createInventoryStore<Row>(options), requests, counts };
}
...
describe("createInventoryStore — sorting, filtering and the project boundary", () => {
  ...
  it("discards the window and recounts when the filter changes", async () => {
    ...
    expect(last(h.requests)?.filter).toEqual({
      column: "kind",
      value: "script",
    });
    expect(last(h.requests)?.cursor).toBeNull();
    expect(last(h.counts)?.filter).toEqual({
      column: "kind",
      value: "script",
    });
    expect(h.store.rows.value).toHaveLength(KEYSET_PAGE_ROWS);
  ...
  it("DISCARDS a response that lands after the window was discarded", async () => {
    ...
    // Signalled the moment the first read is genuinely in flight. Without it
    // the test races its own staging: the first load can be superseded before
    // it ever reaches the reader, and then nothing stale exists to be dropped —
