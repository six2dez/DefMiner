...
//    deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES) by this suite.
//    THIS PARAGRAPH RESTATES NO BOUND OF ITS OWN, deliberately, for the same
...
BYTE_COMPARED_SURFACES), declared further down this file, and is compared to
that output BY BYTES by the suite. Do not hand-edit the span: edit the registry and regenerate.
...
byte-compared to deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES) by this suite.
...
/**
 * THE SURFACES THIS SUITE READS AND BYTE-COMPARES - DECLARED ONCE, READ BY ALL
 * THREE OF THE THINGS THAT WOULD OTHERWISE DISAGREE.
 *
 * `GATE_FILE`, `LEDGER` and the residual generator each resolve their answer
 * FROM this list rather than beside it. The reader that opens one surface, the
 * reader that opens the other, and the TEXT THAT TELLS A READER WHICH TWO ARE
 * OPENED are then three uses of one definition instead of three definitions of
 * one fact. A path written a second time as its own literal somewhere else in
 * this file would be a second definition, and the drift between two definitions
 * of one fact is what this phase has spent twelve rounds finding.
 *
 * THE ORDER IS THE ORDER THE TWO COMPARISONS RUN IN: this gate spec's own
 * header first, the requirement ledger second.
 *
 * WHAT THIS DOES NOT ESTABLISH, WRITTEN HERE RATHER THAN LEFT TO BE FOUND. It
 * says which files are COMPARED, and nothing else. It is not a claim that a
 * file outside it is correct, and not a claim that a file outside it is watched
 * by some other mechanism. A file that is not listed here is reached by THIS
 * comparison not at all - which is a statement about this mechanism's reach and
 * not a verdict on that file.
 */
export const BYTE_COMPARED_SURFACES = Object.freeze([
  "packages/backend/src/outbound-prohibition.spec.ts",
  ".planning/REQUIREMENTS.md",
] as const);
...
export const DERIVED_BEGIN = `BEGIN ${DERIVED_MARK} - generated by the TWO-ARGUMENT residual generator - the registry first, the byte-compared surface set second - in packages/backend/src/outbound-prohibition.spec.ts - MACHINE-OWNED, DO NOT HAND-EDIT`;
 ...
 * dates, no ordering that depends on anything but ITS ARGUMENTS - the registry's
 * own order and the surface set's own order. A generator whose output is not a
 * function of ITS ARGUMENTS alone CANNOT be byte-compared, and the byte
 * comparison is the only thing that makes the shipped text bound to the code
 * rather than adjacent to it.
 *
 * THE SURFACE SET ARRIVES AS A PARAMETER RATHER THAN BEING READ OFF THE MODULE
 * CONSTANT FROM IN HERE, AND THE REASON IS THIS FILE'S OWN RECURRING DEFECT. The
 * BEGIN sentinel names the call a hand-editor has to re-run. A call that quietly
 * grew an input while that sentinel still described the old one would BE the next
 * sentence in this file claiming something the code does not do. Taking the set
 * as an argument puts the arity in the signature, where the sentinel's text and
 * the two call sites are all checked against it by the compiler.
 *
...
export function deriveResidual(
  registry: readonly ResolverRecord[],
...
export function extractDerivedBlock(
  ...
  if (first === -1) {
    throw new SentinelMissingError(
      `the BEGIN sentinel is absent. Looked for a line containing: ${begin}. The generated block is machine-owned; restore the sentinel and paste deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES)'s output between the markers.`,
    );
  }
  if (last === -1) {
    throw new SentinelMissingError(
      `the END sentinel is absent. Looked for a line containing: ${end}. The generated block is machine-owned; restore the sentinel and paste deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES)'s output between the markers.`,
    );
  }
  if (last <= first) {
    throw new SentinelMissingError(
...
describe("the residual is DERIVED — the registry is bound to the walk, and the shipped text to the registry", () => {
  ...
  const GATE_FILE = BYTE_COMPARED_SURFACES[0];
  ...
  it("the gate file read is NON-EMPTY and carries BOTH sentinels — non-vacuity, asserted BEFORE the rule", () => {
    ...
    expect(
      ...
      `${GATE_FILE} no longer contains the BEGIN sentinel. The generated residual block was removed or its marker was edited; restore the marker and regenerate the span with deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES).`,
    ).toBe(true);
    expect(
    ...
    return [
      {
        ...
        proof:
          "THE RESIDUAL OF CORE-11's OUTBOUND WALK - DERIVED, NOT AUTHORED.",
        why: "it IS the derived residual; the phrasings in it are generated from the registry and byte-compared to deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES).",
      },
      {
  ...
  const LEDGER = BYTE_COMPARED_SURFACES[1];
  ...
  it("the block shipped in .planning/REQUIREMENTS.md equals deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES), byte for byte", () => {
    const shipped = extractDerivedBlock(
      readPlanningLedger(LEDGER),
      DERIVED_BEGIN,
      DERIVED_END,
    );
    const generated = deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES);
    expect(
      shipped,
      `the derived residual block in ${LEDGER} DIVERGED from deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES).\n\nThe GENERATED text is authoritative and the shipped text is the defect. Replace the span between the sentinels with exactly this:\n\n----- BEGIN EXPECTED -----\n${generated}\n----- END EXPECTED -----\n`,
    ).toBe(generated);
  });
  ...
  it("the block shipped in the gate header equals deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES), byte for byte", () => {
    const shipped = extractDerivedBlock(gateText, DERIVED_BEGIN, DERIVED_END);
    const generated = deriveResidual(RESOLVER_REGISTRY, BYTE_COMPARED_SURFACES);
    expect(
      shipped,
